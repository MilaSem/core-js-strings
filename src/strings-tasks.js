...
function removeFirstOccurrences(str, value) {
  ...
  const result =
    start !== -1 ? str.substring(0, start).concat(str.substring(end)) : str;
...
function formatTime(minutes, seconds) {
  ...
  const result = `${strMin.padStart(2, '0')}:${strSec.padStart(2, '0')}`;
...
function isPalindrome(str) {
  const newStr = str.replaceAll(/[\s!?,]/gi, '');
  let reverseStr = '';
...
function findLongestWord(sentence) {
  ...
  const arrSortWords = arrWords.sort((a, b) => b.length - a.length);
  return arrSortWords[0];
}
...
function reverseWords(str) {
  ...
  const arrReversStr = arrStr.map((item) => {
    let reverseStr = '';
...
function getCardId(value) {
  ...
  const rate = value.slice(0, value.length - 1);
  switch (rate) {
    case 'A':
      id += 1;
      break;
